...
if(NOT socket_listener_count EQUAL 1)
    message(FATAL_ERROR
        "keysharp-input.socket must declare exactly one ListenStream; found ${socket_listener_count}")
endif()

if(NOT socket_unit MATCHES
    "ListenStream[ \t]*=[ \t]*/run/keysharp-input/keysharp-input\\.sock")
    message(FATAL_ERROR "keysharp-input.socket does not own the expected single endpoint")
endif()

if(NOT socket_unit MATCHES "\\[Install\\]"
    OR NOT socket_unit MATCHES "WantedBy[ \t]*=[ \t]*sockets\\.target")
    message(FATAL_ERROR
        "keysharp-input.socket must remain disableable for clean upgrades")
endif()

if(NOT service_unit MATCHES
    "Requires[ \t]*=[ \t]*keysharp-input\\.socket")
    message(FATAL_ERROR
        "keysharp-input.service must require its socket activation unit")
endif()

if(NOT service_unit MATCHES "\\[Install\\]"
    OR NOT service_unit MATCHES "WantedBy[ \t]*=[ \t]*multi-user\\.target")
    message(FATAL_ERROR
        "keysharp-input.service must be the sole multi-user boot entry point")
endif()

message(STATUS "keysharp-input systemd unit topology passed")
